...
export async function monitMigrationFinish(base_path?: string, rc_port?: string|number, ah_port?: string|number) {
    ...
    let r = await Promise.all([rc_check(rc_uri), ah_check(ah_uri), mock_finish(20*1000, migration_is_mocked)]);
    ...
    await fs_promises.writeFile(`${base_path_to_use}/migration_done.json`, JSON.stringify(content));
...
async function rc_check(uri: string) {
    return new Promise( async (resolve) => {
        ...
        const unsub = await api.query.rcMigrator.rcMigrationStage(async (raw: any) => {
            let stage = raw.toHuman();
            // TODO: remove it
            console.debug("stage", stage);
            const finished = Object.keys(stage)[0] == 'MigrationDone' || mock_finish_flag;
            if(finished) {
...
async function ah_check(uri: string) {
    return new Promise( async (resolve) => {
        const api = await connect(uri);
        const unsub = await api.query.ahMigrator.ahMigrationStage(async (raw: any) => {
            let stage = raw.toHuman();
            // TODO: remove it
            console.debug("stage", stage);
            const finished = Object.keys(stage)[0] == 'MigrationDone' || mock_finish_flag;
